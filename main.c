...
{	
	cfgPorts();
	initLCD();
	unsigned char Cnt_Uni = 0x30;
	unsigned char Cnt_Dec = 0x31;
	unsigned char Cnt_Cen = 0x31;
	for(;;)
	{
		sendCode(nIns, 0x87);
		sendCode(nData, Cnt_Uni);
		delay(n250msec);
		Cnt_Uni++;
		if(Cnt_Uni == 0x3A)
		{
